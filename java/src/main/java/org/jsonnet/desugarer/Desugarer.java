...
public class Desugarer {
   public Desugarer(final Allocator alloc) {
       this.alloc = alloc;
   }
     ...
     * @param tla the top level arguments.  If null then do not try to process top-level functions.
     */
    public static void jsonnet_desugar(final Allocator alloc,
                                       final AST ast/*,
                                                      final Map<String, VmExt> tlas*/)
    {
        final Desugarer desugarer = new Desugarer(alloc);
    ...
    */
    ...
    public void desugar(final AST ast_, unsigned obj_level) {
    ...
    */
    ...
    public void desugarFile(final AST ast/*, Map<String, VmExt> tlas*/) {
    /*
        this.desugar(ast, 0);

        // Now, implement the std library by wrapping in a local construct.
        Tokens tokens = Lexer.jsonnet_lex("std.jsonnet", STD_CODE);
        AST std_ast = Parser.jsonnet_parse(alloc, tokens);
        desugar(std_ast, 0);
    ...
    */
    ...
    public static final byte[] STD_CODE;

    static {
        final InputStream resourceInput = Desugarer.class.getResourceAsStream("/org.jsonnet/std.jsonnet");
        final ByteArrayOutputStream byteOutput = new ByteArrayOutputStream();
        final byte[] buffer = new byte[1024];
        try {
            while (true) {
                final int read = resourceInput.read(buffer);
                if (read < 0) {
                    break;
                }
        ...
        STD_CODE = byteOutput.toByteArray();
    }
